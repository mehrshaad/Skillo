...
| Claude Code | Claude Code installed locally, plus a one-time bridge install. No API key. | see [`bridge/README.md`](bridge/README.md) |
...
3. **Tailor** — optionally add notes ("lean on my Python work", "I led the
   migration"), then *Generate*. Notes are treated as facts about you, so the
   rewrite may use them.
4. **Review** — read the change summary and the diff. Then *Apply to Overleaf*,
   or copy or download the LaTeX. If it is not right, *Regenerate with feedback*
...
node scripts/generate-icons.mjs   # regenerate the icons
```
...
If you find yourself writing a LinkedIn selector outside `jobIntake/`, or an
Overleaf detail outside the Overleaf adapter, put it back.
